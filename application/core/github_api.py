...
async def fetch_repo_metadata(owner: str, repo: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if e.response.status_code == 401:
            raise typer.Exit("Unauthorized: Check your GitHub API Key.")
        elif e.response.status_code == 404:
            raise typer.Exit(f"Repository '{owner}/{repo}' not found.")
        else:
            raise typer.Exit(
...
async def fetch_commits_history(owner: str, repo: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if e.response.status_code == 401:
            ...
            raise typer.Exit(f"Commits not found for repository '{owner}/{repo}'.")
        else:
            raise typer.Exit(
...
async def fetch_contributors(owner: str, repo: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if e.response.status_code == 401:
            ...
            raise typer.Exit(f"Contributors not found for repository '{owner}/{repo}'.")
        else:
            raise typer.Exit(
...
async def fetch_issues(owner: str, repo: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if e.response.status_code == 401:
            ...
            raise typer.Exit(f"Issues not found for repository '{owner}/{repo}'.")
        else:
            raise typer.Exit(
        ...
        raise typer.Exit(f"Failed to fetch issues for repository '{owner}/{repo}': {e}")
...
async def fetch_releases(owner: str, repo: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if e.response.status_code == 401:
            ...
            raise typer.Exit(f"Releases not found for repository '{owner}/{repo}'.")
        else:
            raise typer.Exit(
...
async def fetch_languages(owner: str, repo: str) -> Dict[str, Any]:
    ...
    try:
        ...
        if e.response.status_code == 401:
            ...
            raise typer.Exit(f"Languages not found for repository '{owner}/{repo}'.")
        else:
            raise typer.Exit(
